...
LANL_FOLDER = '../data/lanl_output/tmp/auth_'
...
def load_lanl_dist(workers, start=150885, end=1209600, delta=3600, is_test=True, ew_fn=std_edge_w):
    ...
    # total_edge = 0
    # while i < len(eis):
    #     # tensorflow processing method
    #     # eis[i] = tf.concat([eis[i], tf.expand_dims(ys[i], 0)], 0)
    #     # reshaped_tensor = tf.reshape(tf.transpose(eis[i], [1, 0]), [-1, 3])
    #     # with tf.Session() as sess:
    #     #     eis_numpy = reshaped_tensor.eval()
    #     # eis_list = eis_numpy.tolist()
    #     # new_list = np.reshape(eis_list, (-1, 3))
    #     # print(new_list)
    #     # total_edge += len(eis_list)
    #
    #     # torch processing
    #     eis[i] = torch.cat((eis[i], ys[i].unsqueeze(0)), dim=0)
    #     transposed_tensor = torch.transpose(eis[i], 0, 1)
    #     eis_numpy = transposed_tensor.numpy()
    #     eis_list = eis_numpy.tolist()
    #     new_list = np.reshape(eis_list, (-1, 3))
    #     print(len(new_list))
    #     print(new_list)
...
def load_partial_lanl(start=140000, end=156659, delta=8640, is_test=True, ew_fn=standardized):
...
if __name__ == '__main__':
    data = load_lanl_dist(2, start=150885, end=1209600, delta=3600)
    print(data)
